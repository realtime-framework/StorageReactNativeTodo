...
/**
 * @providesModule RCTRealtimeCloudStorageIOS
 * @flow
 */
...
class RCTRealtimeCloudStorageIOS extends React.Component {
	...
	static RTEventListener(notification, callBack: Function){
		...
		if (channelExists){
			RCTRealtimeCloudStorageIOS.RTRemoveEventListener(notification);
		}
	...
	getTables(success:Function, error:Function){
		...
		RCTRealtimeCloudStorageIOS.RTEventListener(this.sId + "-getTables", function(data){
			if (data.error)
			{
				error(data);
			}else
	...
	isAuthenticated(aAuthenticationToken, success: Function, error: Function)
	{
		...
		RCTRealtimeCloudStorageIOS.RTEventListener(this.sId + "-isAuthenticated", function(data){
			if (data.error)
			{
				error(data);
			}else
...
class tableRef{
	...
	create(aPrimaryKey, aPrimaryKeyDataType, aProvisionType, aProvisionLoad, success: Function, error: Function){
		...
		RCTRealtimeCloudStorageIOS.RTEventListener(this.tId + "-create", function(data){
			if (data.error) {
				error(data);
			}else
			{
	...
	del(success: Function, error: Function){
		...
		RCTRealtimeCloudStorageIOS.RTEventListener(this.tId + "-del", function(data){
			if (data.error) {
				error(data);
			}else
			{
	...
	update(ProvisionType, aProvisionLoad, success: Function, error: Function){
		...
		RCTRealtimeCloudStorageIOS.RTEventListener(this.tId + "-update", function(data){
			if (data.error) {
				error(data);
			}else
			{
	...
	item(primaryKey){
		...
		storage.item(primaryKey, this.sId, this.tId, itemref.iId);
		return itemref;
	}

	itemCustom(primaryKey, secondary){
		var itemref = new itemRef();
		storage.itemCustom(primaryKey, secondary, this.sId, this.tId, itemref.iId);
		return itemref;
	}
	...
	getItems(itemSnapshot: Function, error: Function)
	{
		...
		RCTRealtimeCloudStorageIOS.RTEventListener(this.tId + "-getItems", function(data){
			if (data && data.error) {
				error(data);
			}else
			{
	...
	push(aItem, success:Function, error: Function){
		...
		RCTRealtimeCloudStorageIOS.RTEventListener(this.tId + "-push", function(data){
			if (data.error) {
				error(data);
			}else
			{
	...
	meta(meta, success:Function, error: Function){
		...
		RCTRealtimeCloudStorageIOS.RTEventListener(this.tId + "-meta", function(data){
			if (data.error) {
				error(data);
			}else
			{
	...
	on(eventType, callback: Function){
		...
		RCTRealtimeCloudStorageIOS.RTEventListener(this.tId+"-on-"+eventType, callback);
	}
	...
	off(eventType){
		...
		RCTRealtimeCloudStorageIOS.RTRemoveEventListener(this.tId+"-on-"+eventType);
	}
	...
	once(eventType, callback: Function){
		...
		RCTRealtimeCloudStorageIOS.RTEventListener(this.tId+"-once-"+eventType, callback);
	}
...
class itemRef{
 ...
 module.exports = RCTRealtimeCloudStorageIOS;
